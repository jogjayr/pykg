...
const downloadSource = pypiReleaseInfo => {
  ...
  const fsStream = fs.createWriteStream('/tmp/bs4.tar');
  ...
  extract.on('finish', function() {
    ...
    console.log('in finalize', arguments);
    pack.entry({name: 'package.json'}, JSON.stringify({name: pypiReleaseInfo.info.name, version: pypiReleaseInfo.info.version}), function() {
      console.log('added packagejson');
      pack.finalize()
    } );
  })
